...
//pilih fungsi
	switch($proses){
		...
		case "gol_insert":
			{
				...
				$nama_tabel="golongan";
				$values="'$_POST[id_gol]','$_POST[nama_gol]','$_POST[biaya]'";
				$hal="golongan";
				insert($nama_tabel,$values);
				break;
			}
		...
		{			
			...
			$idMax1 = $data1['maxID'];
			$noUrut1 = (int) substr($idMax1, 4, 4);
			$noUrut1++;
			$newID1 = $jenis1 . sprintf("%03s", $noUrut1);
			$bpen=$_POST['biaya_adm'];
			$bmem=$_POST['biaya_mem'];
			$total=$bpen+$bmem; 
			$lokasi_file=$_FILES[gambar][tmp_name];
			if(empty($lokasi_file)){
			$reg="INSERT INTO registrasi(`no_registrasi`, `id_member`,`nama_member`, `nama_golongan`, `tanggal_pendaftaran`, `biaya_pendaftaran`,`biaya_member`) VALUES ('$_POST[no_reg]','$newID1','$_POST[nama_mem]','$_POST[id_golongan]','$_POST[tgl_reg]', '$_POST[biaya_adm]', '$_POST[biaya_mem]')";
			mysql_query($reg);
			$mem="INSERT INTO member(`id_member`, `nama`, `alamat`, `tempat_lahir`, `tanggal_lahir`, `jenis_kelamin`, `no_telepon`, `status`, `tgl_tempo`, `gambar`) VALUES ('$newID1', '$_POST[nama_mem]', '$_POST[alamat_mem]', '$_POST[tmp_mem]', '$_POST[tgl_mem]', '$_POST[jenis_kelamin]', '$_POST[no_tlpn]', 'aktif', '$_POST[tgl_tempo]', 'member_default.jpg')";
			mysql_query($mem);
			$pem="INSERT INTO pembayaran(`no_transaksi`, `id_member`, `nama`, `nama_golongan`, `tanggal_pembayaran`, `tanggal_jatuh_tempo`, `total_bayar`) VALUES ('$newID', '$newID1', '$_POST[nama_mem]', '$_POST[id_golongan]', '$_POST[tgl_reg]', '$_POST[tgl_tempo]', '$total')";
			mysql_query($pem);	
			...
			$reg="INSERT INTO registrasi(`no_registrasi`, `id_member`,`nama_member`, `nama_golongan`, `tanggal_pendaftaran`, `biaya_pendaftaran`,`biaya_member`) VALUES ('$_POST[no_reg]','$newID1','$_POST[nama_mem]','$_POST[id_golongan]','$_POST[tgl_reg]', '$_POST[biaya_adm]', '$_POST[biaya_mem]')";
			mysql_query($reg);
			$mem="INSERT INTO member(`id_member`, `nama`, `alamat`, `tempat_lahir`, `tanggal_lahir`, `jenis_kelamin`, `no_telepon`, `status`, `tgl_tempo`, `gambar`) VALUES ('$newID1', '$_POST[nama_mem]', '$_POST[alamat_mem]', '$_POST[tmp_mem]', '$_POST[tgl_mem]', '$_POST[jenis_kelamin]', '$_POST[no_tlpn]', 'aktif', '$_POST[tgl_tempo]', '$nama_file')";
			mysql_query($mem);
			$pem="INSERT INTO pembayaran(`no_transaksi`, `id_member`, `nama`, `nama_golongan`, `tanggal_pembayaran`, `tanggal_jatuh_tempo`, `total_bayar`) VALUES ('$newID', '$newID1', '$_POST[nama_mem]', '$_POST[id_golongan]', '$_POST[tgl_reg]', '$_POST[tgl_tempo]', '$total')";
			mysql_query($pem);	
			lompat_ke("pembayaran.php");
		...
		case "gol_update":
			{
				$nama_tabel="golongan";
				$values="nama_golongan='$_POST[nama_gol]', biaya='$_POST[biaya]'";
